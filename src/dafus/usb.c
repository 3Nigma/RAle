...
/* static prototipes definition region */
static void usb_receive(byte_t* data, byte_t rx_len); /* Inspects an incoming packet */
static void usb_transmit(void);                       /* Loads the transmit buffer with the next packet */
...
usb_poll(void) {
  ...
  if(i == 0) {
    ...
    usb_address = 0;
    #ifdef USBTINY_USB_OK_LED
      CLR(USBTINY_USB_OK_LED);
    #endif
  }
}

static void 
usb_receive(byte_t* data, byte_t rx_len) {
  byte_t len;
  byte_t type;
  byte_t limit;
